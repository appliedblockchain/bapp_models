...
module ECC_Crypto
  ...
  class EncryptionWrongPrivateKeyError < EncryptionError
  end
  class DecryptionWrongPrivateKeyError < DecryptionError
  end
  ...
  class ECIES
    ...
    class <<self
      ...
      def encrypt(data, remote_pubkey, shared_mac_data='')
        ...
        # 4. send 0x04 || R || AsymmetricEncrypt(shared-secret, plaintext) || tag
        tag = ECC_Crypto.hmac_sha256 key_mac, "#{iv}#{ciphertext}#{shared_mac_data}"
      ...
      def decrypt(curve, data, shared_mac_data='')
        # this version raises an error if you can't decrypt a value
        # raise DataEmptyDecryptionError, "data not found - curve: #{curve.inspect} - shared_mac_data: #{shared_mac_data.inspect}" if data.empty?
        #
        # this version instead returns a null-object
        return NullObject.new(data: data)  if !data || data.empty?

        raise DecryptionError, "wrong ecies header - data: #{data.inspect}" unless data[0] == "\x04"
        ...
        # 2. verify tag
        raise DecryptionError, 'Fail to verify data' unless ECC_Crypto.hmac_sha256(key_mac, "#{data[65...-32]}#{shared_mac_data}") == tag
