# from https://github.com/cryptape/ruby-devp2p/blob/master/lib/devp2p/crypto/ecc_x.rb
# - crypto - ecies and  utils are all from ecc / ecc_x / devp2p
  ...
  class ECCx
    ...
    class <<self
      def valid_key?(raw_pubkey, raw_privkey=nil)
        ...
        bn = OpenSSL::BN.new ECC_Utils.encode_hex("\x04#{raw_pubkey}"), 16
        point = OpenSSL::PKey::EC::Point.new group, bn

        key = OpenSSL::PKey::EC.new(CURVE)
        key.public_key = point
        key.private_key = OpenSSL::BN.new ECC_Utils.big_endian_to_int(raw_privkey) if raw_privkey
        key.check_key
      ...
      def generate_key
        ...
        raw_privkey = ECC_Utils.zpad ECC_Utils.int_to_big_endian(curve.private_key.to_i), 32
        raw_pubkey = ECC_Utils.int_to_big_endian(curve.public_key.to_bn.to_i)
      ...
      def raw_pubkey_to_openssl_pubkey(raw_pubkey)
        ...
        raw_pubkey = "\x04#{raw_pubkey}"

        bn = OpenSSL::BN.new ECC_Utils.encode_hex(raw_pubkey), 16
        group = OpenSSL::PKey::EC::Group.new CURVE
        begin
          OpenSSL::PKey::EC::Point.new group, bn
        rescue OpenSSL::PKey::EC::Point::Error
          raise "CantConvertPublicKey - Can't convert Public key to EC Public key - Probably the key is null - raw_pubkey: #{raw_pubkey.inspect}"
        end
      end
      ...
      def raw_privkey_to_openssl_privkey(raw_privkey)
        ...
        OpenSSL::BN.new ECC_Utils.big_endian_to_int(raw_privkey)
      end
    end

    attr :raw_pubkey, :raw_privkey
    ...
    def initialize(raw_privkey=nil, raw_pubkey=nil)
      if raw_privkey && raw_pubkey
        ...
        raw_pubkey = ECC_Crypto.privtopub raw_privkey
      elsif raw_pubkey
    ...
    def sign(data)
      sig = ECC_Crypto.ecdsa_sign data, @raw_privkey
